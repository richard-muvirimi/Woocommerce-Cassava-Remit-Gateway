=== WooCommerce Cassava Remit Gateway ===
Contributors:  alexandrubr, Tyganeutronics
Tags: woocommerce gateway, gateway, cassava remit, payment
Requires at least: 4.0.0
Tested up to: 4.9.4
Stable tag: 1.1.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

The plugin helps you accept payments using Cassava Remit Transfers.

== Description ==

The plugin helps you accept payments using Cassava Remit Transfers.

== Installation ==

1. Upload 'woocommerce-cassava-remit-gateway' to the '/wp-content/plugins/' directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Access 'admin.php?page=wc-settings&tab=checkout&section=cassavaremit' in order to change the description and the details provided in the email

== Changelog ==
= 1.1 =
* Added order status settings. You can now choose if you want the order to be set On Hold or Processing status.
* Removed the tooltip and displayed the intructions under each settings. This will help you better understand what each setting does.
* Updated the plugin description.
* Updated the Settings button to get you in the right WooCommerce section.
* The plugin now reduces the stock using the latest code sintax (provided by WooCommerce).

= 1.0 =
* Initial release.